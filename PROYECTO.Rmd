...
```{r Gráfica9}
###Dividiremos la poblacion en tres grupos: niños (de 0 a 14 años), adultos (de 25 a 59 años) y viejos (de 60 años en adelante)
...
for (i in 1:32){
  for (j in 1:11){
    suma_nin[i,j]<-sum(subset(totnot, Estado == estados[i] & Año == 2002+j, 5)[1:4,])
    suma_adu[i,j]<-sum(subset(totnot, Estado == estados[i] & Año == 2002+j, 5)[7:9,])
    suma_vie[i,j]<-sum(subset(totnot, Estado == estados[i] & Año == 2002+j, 5)[10:11,])
  }
}

titulos <- c("Estados", 2003:2013)
total.niños <- as.data.frame(matrix(c(estados, suma_nin), ncol=12,))
